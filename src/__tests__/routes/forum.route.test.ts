...
describe('Forum Routes', () => {
    ...
    describe('GET /api/v1/forum/posts', () => {
        ...
        it('should return 200 and posts array', async () => {
            ...
            expect(response.body.message).toContain('posts found')
        })
        ...
        it('should return 200 with tag filter', async () => {
            const mockPosts = [createMockPost({ tags: [createMockTag()] })]
            prismaMock.post.findMany.mockResolvedValue(mockPosts)
        ...
        it('should return 200 with category filter', async () => {
            const mockPosts = [createMockPost({ category: 'health' })]
            prismaMock.post.findMany.mockResolvedValue(mockPosts)
        ...
        it('should return 200 with search filter', async () => {
            const mockPosts = [createMockPost({ title: 'Search Test' })]
            prismaMock.post.findMany.mockResolvedValue(mockPosts)
        ...
        it('should return 200 with newest filter', async () => {
            const mockPosts = [createMockPost()]
            prismaMock.post.findMany.mockResolvedValue(mockPosts)
        ...
        it('should return 200 with popular filter', async () => {
            const mockPosts = [createMockPost({ views: 100 })]
            prismaMock.post.findMany.mockResolvedValue(mockPosts)
        ...
        it('should return 200 with hot filter', async () => {
            const mockPosts = [createMockPost()]
            prismaMock.post.findMany.mockResolvedValue(mockPosts)
        ...
        it('should return 200 with unanswered filter', async () => {
            const mockPosts = [createMockPost({ _count: { replies: 0 } })]
            prismaMock.post.findMany.mockResolvedValue(mockPosts)
        ...
        it('should return 404 when no posts found', async () => {
            prismaMock.post.findMany.mockResolvedValue(null as unknown as never[])

            const response = await supertest(App)
                .get(postsEndpoint)

            expect(response.status).toBe(404)
            expect(response.body.error[0].statusType).toBe('Not Found')
        })
    ...
    describe('POST /api/v1/forum/posts', () => {
        ...
        it('should return 200 for valid post creation', async () => {
        ...
        it('should return 403 for missing title', async () => {
            ...
            expect(response.status).toBe(403)
            expect(response.body.error[0].property).toBe('title')
        })
        ...
        it('should return 403 for missing body', async () => {
            ...
            expect(response.status).toBe(403)
            expect(response.body.error[0].property).toBe('body')
        })

        it('should return 403 for missing category', async () => {
            const mockUser = createMockUser()
            const { token, csrfSecret, csrfToken } = createAuthenticatedRequest(mockUser)
            ...
            expect(response.status).toBe(403)
            expect(response.body.error[0].property).toBe('category')
        })

        it('should return 403 for missing tags', async () => {
            const mockUser = createMockUser()
            const { token, csrfSecret, csrfToken } = createAuthenticatedRequest(mockUser)
            ...
            expect(response.status).toBe(403)
            expect(response.body.error[0].property).toBe('tags')
        })
    })
    ...
    describe('GET /api/v1/forum/posts/:postId', () => {
        it('should return 200 and single post', async () => {
            ...
            expect(response.status).toBe(200)
            expect(response.body.data.id).toBe('test-post-id-123')
        })
    ...
    describe('PUT /api/v1/forum/posts/:postId', () => {
        it('should return 200 for valid update by owner', async () => {
            const mockUser = createMockUser()
            const mockPost = createMockPost({ authorId: mockUser.id })
            const { token, csrfSecret, csrfToken } = createAuthenticatedRequest(mockUser)

            prismaMock.post.findUnique.mockResolvedValue(mockPost)
            prismaMock.post.update.mockResolvedValue({ ...mockPost, title: 'Updated Title' })
        ...
        it('should return 401 for non-owner', async () => {
            const mockUser = createMockUser()
            const mockPost = createMockPost({ authorId: 'different-user-id' })
            const { token, csrfSecret, csrfToken } = createAuthenticatedRequest(mockUser)
    ...
    describe('DELETE /api/v1/forum/posts/:postId', () => {
        it('should return 200 for valid delete by owner', async () => {
            const mockUser = createMockUser()
            const mockPost = createMockPost({ authorId: mockUser.id })
            const { token, csrfSecret, csrfToken } = createAuthenticatedRequest(mockUser)
            ...
            const response = await supertest(App)
                .delete('/api/v1/forum/posts/test-post-id-123')
                .set('Cookie', [`accessToken=${token}`, `_csrf=${csrfSecret}`])
                .set('x-csrf-token', csrfToken)
    ...
    describe('GET /api/v1/forum/posts/:postId/reply', () => {
        it('should return 200 and replies array', async () => {
            ...
            prismaMock.reply.findMany.mockResolvedValue(mockReplies)

            const response = await supertest(App)
                .get('/api/v1/forum/posts/test-post-id-123/reply')

            expect(response.status).toBe(200)
            expect(response.body.data).toBeInstanceOf(Array)
        })

        it('should return 404 when no replies found', async () => {
            prismaMock.reply.findMany.mockResolvedValue(null as unknown as never[])
    ...
    describe('POST /api/v1/forum/posts/:postId/reply', () => {
        it('should return 200 for valid reply creation', async () => {
            const mockUser = createMockUser()
            ...
            const response = await supertest(App)
                .post('/api/v1/forum/posts/test-post-id-123/reply')
                .set('Cookie', [`accessToken=${token}`, `_csrf=${csrfSecret}`])
                .set('x-csrf-token', csrfToken)
                .send({
    ...
    // ==================== UPDATE REPLY ====================
    describe('PUT /api/v1/forum/posts/:postId/reply/:replyId', () => {
        it('should return 200 for valid update by owner', async () => {
            const mockUser = createMockUser()
            const mockReply = createMockReply({ authorId: mockUser.id })
            const { token, csrfSecret, csrfToken } = createAuthenticatedRequest(mockUser)

            prismaMock.reply.findUnique.mockResolvedValue(mockReply)
            prismaMock.reply.update.mockResolvedValue({ ...mockReply, body: 'Updated reply' })

            const response = await supertest(App)
                .put('/api/v1/forum/posts/test-post-id-123/reply/test-reply-id-123')
                .set('Cookie', [`accessToken=${token}`, `_csrf=${csrfSecret}`])
                .set('x-csrf-token', csrfToken)
                .send({
                    body: 'Updated reply'
        ...
        it('should return 401 for non-owner', async () => {
            const mockUser = createMockUser()
            const mockReply = createMockReply({ authorId: 'different-user-id' })
            const { token, csrfSecret, csrfToken } = createAuthenticatedRequest(mockUser)
        ...
        it('should return 404 for non-existent reply', async () => {
            ...
            const response = await supertest(App)
                .put('/api/v1/forum/posts/test-post-id-123/reply/non-existent-id')
                .set('Cookie', [`accessToken=${token}`, `_csrf=${csrfSecret}`])
                .set('x-csrf-token', csrfToken)
                .send({
                    body: 'Updated reply'
    ...
    // ==================== DELETE REPLY ====================
    describe('DELETE /api/v1/forum/posts/:postId/reply/:replyId', () => {
        it('should return 200 for valid delete by owner', async () => {
            const mockUser = createMockUser()
            const mockReply = createMockReply({ authorId: mockUser.id })
            const { token, csrfSecret, csrfToken } = createAuthenticatedRequest(mockUser)
            ...
            const response = await supertest(App)
                .delete('/api/v1/forum/posts/test-post-id-123/reply/test-reply-id-123')
                .set('Cookie', [`accessToken=${token}`, `_csrf=${csrfSecret}`])
                .set('x-csrf-token', csrfToken)
    ...
    describe('GET /api/v1/forum/tags', () => {
        ...
        it('should return 200 with search filter', async () => {
            const mockTags = [createMockTag({ name: 'javascript' })]
            prismaMock.tag.findMany.mockResolvedValue(mockTags)
        ...
        it('should return 404 when no tags found', async () => {
            prismaMock.tag.findMany.mockResolvedValue(null as unknown as never[])
    ...
    describe('GET /api/v1/forum/tags/:tagId', () => {
        it('should return 200 and single tag', async () => {
            ...
            expect(response.status).toBe(200)
            expect(response.body.data.id).toBe('test-tag-id-123')
        })
