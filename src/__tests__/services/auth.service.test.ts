...
describe('Auth Service', () => {
    ...
    describe('comparePassword', () => {
        it('should return true for valid password match', () => {
            const password = 'TestPassword123!'
            const hashed = hashPassword(password)
        ...
        it('should return false for invalid password match', () => {
            const password = 'TestPassword123!'
    ...
    describe('generateCSRFToken', () => {
        it('should return csrfSecret and csrfToken', () => {
        ...
        it('should generate different tokens on each call', () => {
            ...
            expect(result1.csrfSecret).not.toBe(result2.csrfSecret)
    ...
    describe('getCookiesOptions', () => {
        ...
        it('should return longer or equal maxAge when remember is true', () => {
            const rememberOptions = getCookiesOptions(true)
            const noRememberOptions = getCookiesOptions(false)

            expect(rememberOptions.maxAge).toBeGreaterThanOrEqual(noRememberOptions.maxAge as number)
        })
    ...
    describe('sanitizeUserData', () => {
        ...
        it('should remove resetPasswordOTP from user data', () => {
            const mockUser = createMockUser({ resetPasswordOTP: 123456 })

            const sanitized = sanitizeUserData(mockUser)

            expect(sanitized).not.toHaveProperty('resetPasswordOTP')
        })

        it('should remove resetPasswordExpiration from user data', () => {
            const mockUser = createMockUser({ resetPasswordExpiration: new Date() })

            const sanitized = sanitizeUserData(mockUser)

            expect(sanitized).not.toHaveProperty('resetPasswordExpiration')
        })
        ...
        it('should keep public fields', () => {
            ...
            expect(sanitized.firstName).toBe(mockUser.firstName)
            expect(sanitized.lastName).toBe(mockUser.lastName)
    ...
    describe('generateRandomUsername', () => {
        ...
        it('should generate different usernames on each call', () => {
    ...
    describe('verifyResetPasswordOTP', () => {
        it('should return true for valid OTP and not expired', () => {
            const futureDate = new Date(Date.now() + 1000 * 60 * 60)
            const otp = 123456

            const result = verifyResetPasswordOTP(otp, futureDate, otp)
        ...
        it('should return false for wrong OTP', () => {
            ...
            const result = verifyResetPasswordOTP(123456, futureDate, 654321)
        ...
        it('should return false for expired OTP', () => {
            ...
            const result = verifyResetPasswordOTP(otp, pastDate, otp)

            expect(result).toBe(false)
        })

        it('should handle string OTP by converting to number', () => {
            const futureDate = new Date(Date.now() + 1000 * 60 * 60)
            const otp = 123456

            const result = verifyResetPasswordOTP(otp, futureDate, '123456' as unknown as number)
    ...
    describe('login', () => {
        ...
        it('should throw AuthError for non-existent user', async () => {
            ...
            await expect(login('notfound@test.com', 'Password123!'))
                .rejects
