...
describe('Forum Service', () => {
    ...
    describe('validateOwner', () => {
        it('should not throw for valid post owner', async () => {
            const mockPost = createMockPost({ authorId: 'user-123' })
            prismaMock.post.findUnique.mockResolvedValue(mockPost)

            await expect(validateOwner('post', 'post-id', 'user-123'))
                .resolves
                .not.toThrow()
        })

        it('should throw error for non-owner of post', async () => {
        ...
        it('should not throw for valid reply owner', async () => {
            const mockReply = createMockReply({ authorId: 'user-123' })
            prismaMock.reply.findUnique.mockResolvedValue(mockReply)

            await expect(validateOwner('reply', 'post-id', 'user-123', 'reply-id'))
                .resolves
                .not.toThrow()
        })

        it('should throw error for non-owner of reply', async () => {
            const mockReply = createMockReply({ authorId: 'different-user' })
            prismaMock.reply.findUnique.mockResolvedValue(mockReply)

            await expect(validateOwner('reply', 'post-id', 'user-123', 'reply-id'))
                .rejects
                .toThrow('you are not the author of this reply!')
        })

        it('should throw error for missing replyId when validating reply', async () => {
            await expect(validateOwner('reply', 'post-id', 'user-123'))
                .rejects
    ...
    describe('getPosts', () => {
        ...
        it('should return single post when id provided', async () => {
            const mockPost = createMockPost()
            prismaMock.post.findUnique.mockResolvedValue(mockPost)

            const result = await getPosts(undefined, 'post-id')
        ...
        it('should pass query parameters', async () => {
            ...
            await getPosts({ limit: 5, page: 1, filter: 'newest' })
    ...
    describe('getPostsCount', () => {
        ...
        it('should filter by query', async () => {
            ...
            const result = await getPostsCount({ category: 'health' })
    ...
    describe('updatePost', () => {
        it('should update post', async () => {
            ...
            const result = await updatePost('post-id', { title: 'Updated Title' })
        ...
        it('should handle tag changes', async () => {
            const mockPost = createMockPost()
            const existingTags = [createMockTag({ name: 'old-tag' })]
            prismaMock.tag.findMany.mockResolvedValue(existingTags)
            prismaMock.post.update.mockResolvedValue(mockPost)
    ...
    describe('getTags', () => {
        ...
        it('should return popular tags when filter is popular', async () => {
            const mockTags = [createMockTag()]
            prismaMock.tag.findMany.mockResolvedValue(mockTags)

            const result = await getTags({ filter: 'popular', limit: 10 })
    ...
    describe('updateReply', () => {
        it('should update reply', async () => {
            ...
            prismaMock.reply.update.mockResolvedValue(mockReply)

            const result = await updateReply('reply-id', 'post-id', { body: 'Updated reply' })
    ...
    describe('deleteReply', () => {
        it('should delete reply', async () => {
            ...
            expect(prismaMock.reply.delete).toHaveBeenCalledWith({
                where: { id: 'reply-id', postId: 'post-id' }
            })
        })
    })
})
